...
namespace TareasMVC.Controllers
{
    ...
    public class PasosController: ControllerBase
    {
            ...
            [FromBody] PasoCrearDTO pasoCrearDTO)
        {
            ...
            if (tarea is null) // Si la tarea no existe devuelve la vista de No Encontrado
            {
                return NotFound();
            }

            if (tarea.UsuarioCreacionId != usuarioId) // Si la tarea existe, pero no coinciden los id del usuario que intenta acceder, y el id del creador de la tarea, entonces no tiene permisos para consultarlo.
            {
                return Forbid();
            }
            ...
            [FromBody] PasoCrearDTO pasoCrearDTO)
        {
            ...
            if (paso is null) // Si el paso no existe devuelve la vista de No Encontrado
            {
                return NotFound();
            }

            if (paso.Tarea.UsuarioCreacionId != usuarioId) // Si el paso existe, pero no coinciden los id del usuario que intenta acceder, y el id del creador de la tarea, entonces no tiene permisos para consultarlo.
            {
                return Forbid();
            }
        ...
        public async Task<IActionResult> Ordenar(int tareaId, 
            [FromBody] Guid[] ids)
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            ...
            var pasos = await context.Pasos
                .Where(x => x.TareaId == tareaId)
                .ToListAsync();

            var pasosIds = pasos.Select(x => x.Id);

            var idsPasosNoPertenecenALaTarea = pasosIds.Except(pasosIds).ToList();
